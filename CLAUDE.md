...
| `venta_local` no persiste `total` (`DATA-015`) | **Abierto, mitigado en Fase 7 — acotado a `total` en la Fase 8E** (el hallazgo original también listaba `tipoQuesoNombre`; era falso, `DetalleVentaViewModel` ya lo resuelve vía `tipo_queso_cache` apenas se captura, sin esperar al servidor). `total` es columna `GENERATED ALWAYS` de Postgres, de solo lectura. `VentaDetalle.total` lo modela `nullable` y `V-03` lo muestra como "No disponible" hasta confirmar contra el servidor — **nunca** un cálculo local `cantidad × precioUnitario` | `V-01`/`V-03` (Fase 7) y cualquier pantalla futura que liste `Venta` desde `venta_local`. No bloquea. Documentado en `MOBILE_DATA_MAPPING.md §10` desde la Fase 8B |
| Sin forma de resolver la `zonaId` del ACOPIADOR autenticado (`DATA-016`) | **Abierto, mitigado en Fase 8A/8B** — el JWT solo lleva `sub`/`rol`/`usuarioId` (§4 de este documento) y no existe un endpoint tipo `/api/usuarios/me`; `GET /api/zonas/{zonaId}/ruta` (`A-01`) y `GET /api/comunicados/zona/{zonaId}` (`S-06`) exigen `zonaId` como path param. `A-01` (Fase 8A, aprobado por el usuario): heurística sobre `unidad_cache` — la `Unidad` cuya `responsableId` coincide con el usuario de la sesión activa (`ObtenerZonaAsignadaUseCase`); si hay 0 o más de una zona distinta, devuelve `null` y `A-01` muestra "no se pudo determinar tu zona" — nunca inventa cuál mostrar. `S-06` (Fase 8B): evita el problema por completo leyendo `comunicado_cache`/`comunicado_zona_cache` (poblados por `/sync/cambios`, sin `zonaId`) en vez de llamar al endpoint por zona directo | `A-01` (Fase 8A), `S-06` (Fase 8B). Se cierra si el backend expone la zona del usuario (JWT o endpoint nuevo); mientras tanto, la heurística de `A-01` asume que cada ACOPIADOR es responsable de exactamente una `Unidad`, premisa no verificada contra el modelo de datos real. Documentado en `MOBILE_DATA_MAPPING.md §10` desde la Fase 8B |
| Cámara, ubicación y permisos de iOS sin verificar en runtime (`EscanerQr.ios.kt`, `ProveedorUbicacion.ios.kt`, `SolicitanteDePermiso.ios.kt`) | **Abierto, aceptado a propósito en el checkpoint de Fase 8A** — mismo criterio que el Keychain de la Fase 3 y el `NativeSqliteDriver` de la Fase 4: en Windows solo se pudo confirmar que compilan (`compileKotlinIosArm64`/`IosSimulatorArm64` y sus `Test` en verde). Es la primera capacidad del proyecto que no se puede verificar ni parcialmente en Windows — cámara y GPS reales solo se prueban en un simulador o dispositivo. El riesgo más alto documentado es la retención de `manager`/`delegado` de `CLLocationManager` (ver comentarios en el propio archivo) | Bloquea la confianza real en `A-02`/`A-04` en iOS hasta que `verificacion-ios.yml` corra en verde sobre esos archivos y, más allá de CI (que no ejerce hardware real), alguien lo pruebe en un simulador o dispositivo con una Mac |
