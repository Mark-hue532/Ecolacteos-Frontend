...
| Sin forma de resolver la `zonaId` del ACOPIADOR autenticado (`DATA-016`) | **Abierto, mitigado en Fase 8A** — el JWT solo lleva `sub`/`rol`/`usuarioId` (§4 de este documento) y no existe un endpoint tipo `/api/usuarios/me`; `GET /api/zonas/{zonaId}/ruta` (`A-01`) exige `zonaId` como path param. Mitigación aprobada por el usuario: heurística sobre `unidad_cache` — la `Unidad` cuya `responsableId` coincide con el usuario de la sesión activa (`ObtenerZonaAsignadaUseCase`). Si hay 0 o más de una zona distinta entre esas `Unidad`, se devuelve `null` y `A-01` muestra "no se pudo determinar tu zona" — nunca inventa cuál mostrar | `A-01` (Fase 8A). Se cierra si el backend expone la zona del usuario (JWT o endpoint nuevo); mientras tanto, la heurística asume que cada ACOPIADOR es responsable de exactamente una `Unidad`, premisa no verificada contra el modelo de datos real. Documentar en `MOBILE_DATA_MAPPING.md §10` sigue pendiente |
